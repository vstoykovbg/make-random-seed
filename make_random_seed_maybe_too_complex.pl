...
for ( my $i = 0 ; $i < $howmanyarrays ; $i++ ) {

    @tmp_array = ();

    for ( my $m = 0 ; $m < $howmanywords ; $m++ ) {
        push( @tmp_array, $words[ irand($max_words) ] );
    }
...
my $myseed;
my $really_random;
my $really_random_mod;
my $line;

for ( my $i = 0 ; $i < $howmanywords ; $i++ ) {
    $really_random = makerandom( Size => 10, Strength => 1 );
    print "Please enter random number (or expression) and press Enter: $really_random + ";
    $line              = <STDIN>;
    $really_random     = $really_random + int( eval($line) );
    $really_random_mod = $really_random % $howmanyarrays;
    $myseed .= $seed[$really_random_mod][$i];
    $myseed .= " ";
}
